...
public class DialogBookARoom extends JDialog
{
    ...
    public DialogBookARoom(JFrame parent, ArrayList<Room> rooms)
    {
        ...
        this.roomsTable = new JTable(new DefaultTableModel(new Object[0][0], new String[] { "Name", "Capacity", "Netbeans", "Java", "Windows10", "Ubuntu" }));
    ...
    private void showAllRooms()
    {
        ...
        for(int i = 0; i < rooms.size(); i++)
        {
            ...
            if(room.hasJava())
                hasJava = "yes";
            if(room.hasWindows10())
                hasWindows10 = "yes";
            if(room.hasUbuntu())
                hasUbuntu = "yes";
            
            ((DefaultTableModel)this.roomsTable.getModel()).addRow(new String[] { room.getName(), room.getCapacity() + "", hasNetBeans, hasJava, hasWindows10, hasUbuntu });
        }
    }
                    ...
                    if(time.equals("Any"))
                    {
                        ...
                        for(int j = 0; j < reservations.size(); j++)
                        {
                            ...
                            if(reservation.getDate().equals(date))
                            {
                                ...
                                if(reservation.getTime().equals("8:00 AM to 10:00 AM"))
                                    eightTaken = true;
                                else if(reservation.getTime().equals("11:00 AM to 1:00 AM"))
                                    nineTaken = true;
                                else if(reservation.getTime().equals("2:00 AM to 4:00 PM"))
                                    elevenTaken = true;
                                else if(reservation.getTime().equals("4:00 AM to 5:00 PM"))
                                    oneTaken = true;
                                else if(reservation.getTime().equals("5:00 to 6:00 PM"))
                                    twoTaken = true;
                            }
                        }
